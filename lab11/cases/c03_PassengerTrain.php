...
<body>
    ...
    <form action="c03_PassengerTrain.php" method="get" enctype="application/x-www-form-urlencoded">
        <p><input type="text" name="distance"
                  value="<?php if (!empty($_GET['distance'])) echo $_GET['distance']; else echo 0 ?>"/> Distance (in
            miles)</p>
        <p><input type="text" name="stops"
                  value="<?php if (!empty($_GET['stops'])) echo $_GET['stops']; else echo 0 ?>"/> Number of Stops</p>
        <p><input type="radio" name="weather"
                  value="good" <?php if (empty($_GET['weather']) || $_GET['weather'] == "good") echo 'checked=\"checked\"' ?> />
            Weather is Good
            <input type="radio" name="weather"
                   value="bad" <?php if (!empty($_GET['weather']) && $_GET['weather'] == "bad") echo 'checked=\"checked\"' ?> />
            Weather is Bad</p>
